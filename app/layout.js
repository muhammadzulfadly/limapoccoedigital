...
export default function RootLayout({ children }) {
  return (
    <html lang="id">
      <body className="pt-16"> {/* Tambahkan padding top agar konten tidak tertutup header */}
        ...
        <header className="bg-[#2DB567] fixed top-0 left-0 w-full z-50 flex items-center justify-between px-8 py-3 shadow">
          ...
          <div className="flex items-center gap-4">
            <Link href="/login" className="text-white text-sm underline hover:text-gray-100">
              Masuk
            </Link>
            <Link
              href="/register"
              className="bg-white text-black text-sm font-medium px-4 py-1.5 rounded hover:bg-gray-200"
            >
              Daftar
            </Link>
          </div>
