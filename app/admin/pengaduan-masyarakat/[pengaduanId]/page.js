...
export default function DetailPengaduanPage() {
  ...
  const handleSelesaikan = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`/api/admin/pengaduan-masyarakat/${pengaduanId}/approved`, {
        method: "PUT",
        headers: {
      ...
      <div className="bg-white rounded-lg shadow-md p-8 max-w-4xl mx-auto">
        ...
        <div className="flex justify-between mt-10">
          ...
          {pengaduan.response && pengaduan.status !== "approved" ? (
            ...
            <button onClick={() => router.push(`/admin/pengaduan-masyarakat/${pengaduan.id}/respon`)} className="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700">
              Selanjutnya
            </button>
          ) : null}
        </div>
